...
4. **Application-local (in-process) cache.** A cache *inside* the app process's own memory (a hash map, Guava/Caffeine, an LRU dict). **Fastest of the server-side options (~100 ns, no network)**, but it is **private to one instance** — Azure warns that with many app instances each holds its *own* copy, so they "can quickly become inconsistent" and you may need shorter TTLs or a shared cache instead. `[S2]` This is Lesson 03's multi-copy divergence in miniature: N app instances = N independent caches that drift.

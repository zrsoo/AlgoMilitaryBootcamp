...
- **Brute force:** <For each node's `random` target, linearly scan the original list to find its index, then walk the copy to the same index and point there. O(n²) time, O(n) space (just the copy).>

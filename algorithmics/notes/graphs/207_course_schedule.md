...
- **Brute force:** <DFS from each node detecting a back-edge (cycle) using a recursion-stack set — O(V·(V+E))>
...
- **Key insight:** <All courses finishable ⇔ graph is a DAG; Kahn's dequeues exactly the acyclic nodes, so `count == numCourses` iff no cycle>
- **Edge cases I had to handle:** <edge direction is prereq[1] → prereq[0]; only zero-indegree courses seed the queue; a cycle leaves count < numCourses>

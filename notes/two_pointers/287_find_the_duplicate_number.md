...
- **Optimized:** <Think of the array as a linked list. The next node from node i is nums[i]. Use Floyd's Tortoise and Hare to detect cycle. Initialize a slow pointer, moving one step at at time, and a fast one, moving 2 steps at a time. They both start from 0. Record first point they meet at. Then, set fast to 0 again and move both pointers one step at a time until they meet again. The index (node) at which they meet again is the entry point to the cycle> — O(n) time, O(1) space
